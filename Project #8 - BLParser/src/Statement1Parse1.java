...
public final class Statement1Parse1 extends Statement1 {
    ...
    private static void parseIf(Queue<String> tokens, Statement s) {
        ...
        Reporter.assertElseFatalError(Tokenizer.isCondition(tokens.front()),
                "Error: Not a vaild condition!");

        //No Error

        Condition condition = parseCondition(tokens.dequeue());
        Statement statement = s.newInstance();
        statement.parseBlock(tokens);

        if (tokens.front().equals("ELSE")) {
            tokens.dequeue();
            Statement statementElseBlock = s.newInstance();
            statementElseBlock.parseBlock(tokens);
            s.assembleIfElse(condition, statement, statementElseBlock);
    ...
    private static void parseCall(Queue<String> tokens, Statement s) {
        ...
        String currentCall = tokens.dequeue();
        s.assembleCall(currentCall);
    ...
    public void parse(Queue<String> tokens) {
        ...
        Statement parseStatement = this.newInstance();

        if (Tokenizer.isIdentifier(tokens.front())) {

            if (tokens.front().equals("IF")) {
                parseIf(tokens, parseStatement);
            }

            if (tokens.front().equals("WHILE")) {
                parseWhile(tokens, parseStatement);
            }

            if (tokens.front().equals("CALL")) {

                parseCall(tokens, parseStatement);
            }
        }
