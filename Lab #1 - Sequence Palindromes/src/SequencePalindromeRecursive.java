...
/**
 * Check if a given {@code Sequence<Integer>} is a palindrome. Recursive style 
 * 
 * @author Henry Zhang
 * 
 */
public final class SequencePalindromeRecursive {

    /**
     * Private constructor so this utility class cannot be instantiated.
     */
    private SequencePalindromeRecursive() {
    }
    ...
    private static boolean isPalindrome(Sequence<Integer> s) {
        ...
        if (s.length() == 0 || s.length() == 1) {
        	...
        	int firstNum = s.remove(0);
        	int secondNum = s.remove(s.length() - 1);
